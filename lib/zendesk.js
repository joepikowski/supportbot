...
Zendesk.prototype.chatRequest = function(Input){
    this.set({
...
Zendesk.prototype.chatRouteRequest = function(Input){
    //Checking Inbox
    if (Input.fullText == ""){
        var callback = this.chatInboxCheck.bind(this);
    ...
    //Dealing with a Specific Case
        ...
        if (["pass","take"].indexOf(Input.command) > -1){
            ...
            var callback = this.chatHandleMove.bind(this,command);
            ...
            var caseID = Input.case;
            var callback = this.chatGetRequester.bind(this);
    ...
    //Dealing with a Specific Organization (Unknown)
        ...
        var status = Input.status;
        var callback = this.chatOrgSearch.bind(this,status);
...
Zendesk.prototype.chatHandleMove = function(command,err,ticket){
    if (err){
...
Zendesk.prototype.chatOrgSearch = function(status,err,organizations){
    if (err){
        this.setOne("output","Bad Zendesk Response :(");
        this.chatOutputResult();
    }else{
...
Zendesk.prototype.chatInboxCheck = function(err,results){
    if (err){
        this.setOne("output","Bad Zendesk Response :(");
        this.chatOutputResult();
    }else{
        ...
        if (results.length < 1){
            ...
            this.chatGetRequester(null,results,false);
        }
    }
};

Zendesk.prototype.chatAutoInboxCheck = function(err,results){
    if (err){
        ...
        if (results.length >= 1){
            this.chatGetRequester(null,results,true);
        }
    }
};

Zendesk.prototype.chatGetRequester = function(err,results,automated){
    if (err){
        this.setOne("output","Bad Zendesk Response :(");
        this.chatOutputResult();
    }else{
        if (results.length < 1){
            ...
            function getUser(elem,ind,arr){
                ...
                var caseInfo = elem;
                var callback = this.chatAssembleCases.bind(this,caseInfo,automated);
...
Zendesk.prototype.chatAssembleCases = function(caseInfo,automated,err,requester){
    if (err){
        this.setOne("output","Bad Zendesk Response :(");
    }else{
        var c = caseInfo;
...
Zendesk.prototype.chatOutputResult = function(){
