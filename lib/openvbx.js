...
OpenVBX.prototype.ircRouteRequest = function(Input){
    ...
    if (Input.command == "on"){
        ...
        this.setOne("output","No Monitor Yet :( ... Specify 'On' or 'Off'");
    ...
    function makeRequest(method,reverse,elem,ind,arr){
        var vbxID = elem.vbx;
        var name = elem.name;
        var callback = this.ircLogResult.bind(this,method,name,reverse);
        this.outputLimit++;
...
OpenVBX.prototype.ircLogResult = function(method,name,reverse,err,results){
    ...
    if (this.requestComplete()){
        this.ircAssembleResult();
    }
};

OpenVBX.prototype.ircAssembleResult = function(){
    if (this.command === "off"){
        var command = "Remove";
